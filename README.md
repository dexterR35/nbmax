...
- An introduction retaining its original six-image character hero, followed by an embedded blue/navy loyalty-page example with desktop/mobile preview controls. The existing main film plays directly inside the example, behind a new Max poster. `#intro/film` and `#intro/example` link directly to these sections. An annotated `build-front` promotion study retains six selectable garment leader lines. Four large, paired-image outfit cards—including Emerald—link directly to their complete Character Build or wardrobe-detail pages.
...
The four selected WebP attachments were verified byte-for-byte against the existing display images; those exact images remain in the wardrobe cards without retouching. The neon-blue Character film poster is a built-in ImageGen proposal based on `build-front.webp` and the supplied blue MAX background reference. Its lossless original is `dist/assets/originals/intro-max-blue.png`, the optimized display image is `dist/assets/images/intro-max-blue.webp`, and its complete prompt is recorded in `dist/assets/generation/intro-max-blue.txt`. The later navy reference banner is used in the introduction website presentation, while the neon-blue artwork is used on the Character film page; the original six-image hero and existing video footage are unchanged. Official rank assignments remain in the wardrobe explorer.

The automated checks validate source/render contracts. The refreshed introduction was also reviewed in a local browser at desktop and phone widths, including its video playback and desktop/mobile example switch. This is not a full assistive-technology audit of every exhibition route. Responsive rules cover desktop, tablet and mobile.
...
## NetBet reference presentation update

The original six-image introduction hero remains unchanged. Below it, the Motion / Character film section now uses a NetBet-style site frame with the supplied-reference navy background, a full-body male Max, programme navigation, benefit icons and six official tier badges. A blue/violet circular play control launches the existing main film inside the frame. The Character motion page uses the same circular control on a neon-blue Max poster. The desktop/mobile preview switch remains available.

The new navy banner and its exact built-in ImageGen prompt are saved at `dist/assets/images/intro-max-navy.webp`, `dist/assets/originals/intro-max-navy.png`, and `dist/assets/generation/intro-max-navy.txt`. The earlier neon-blue artwork remains the Character film poster. Neither generated still changes the existing video footage.
